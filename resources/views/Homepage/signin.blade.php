...
<main class="flex-grow flex items-center justify-center bg-gray-200 bg-opacity-50"
   ...
   <!-- Error message -->
@if ($errors->any())
<div id="errorDisplay" class="relative z-10 mb-6 bg-red-500 text-white rounded-md p-4 shadow-lg">
    <div class="flex justify-between items-center">
        ...
        <!-- Close Button -->
        <button id="closeErrorDisplay" class="text-white text-lg font-bold">&times;</button>
    </div>
</div>
@endif
        ...
        <div class="bg-gray-200 rounded-lg shadow-lg">
            ...
            <div class="px-6 py-6 lg:px-8">
                ...
                <form id="signInForm" class="space-y-6" method="POST" action="{{route('login')}}">
                    @csrf
                    <div>
                        <input type="email" name="email" id="email" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-full focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Email" value="{{ old('email') }}" required>
                        @error('email')
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const closeErrorButton = document.getElementById('closeErrorDisplay');
        const errorDisplay = document.getElementById('errorDisplay');
        
        if (closeErrorButton) {
            closeErrorButton.addEventListener('click', function() {
                errorDisplay.style.display = 'none'; // Hide the error message container
            });
        }
    });
</script>
